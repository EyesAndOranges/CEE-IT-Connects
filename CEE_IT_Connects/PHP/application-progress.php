...
<style>
  ...
  .container { max-width: 720px; margin: 0 auto; padding: 28px 16px 60px; margin-top: 70px }
  ...
  .progress-bar-steps { display: flex; align-items: flex-start; justify-content: space-between; position: relative; margin-bottom: 10px; }
  ...
  .progress-step.done:not(:last-child)::after { background: var(--brand); }
  .progress-circle {
    width: 34px; height: 34px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 13px; z-index: 1; position: relative;
  }
  .is-done { background: var(--brand); color: white; }
  .is-active { background: white; border: 2px solid var(--brand); color: var(--brand); }
  .is-locked { background: white; border: 2px solid var(--gray); color: var(--gray-dark); }
  .progress-label { font-size: 14px; margin-top: 20px; text-align: center; }
  .progress-step.active .progress-label { color: var(--brand); font-weight: 500; }
  ...
  .step-header:hover { background: var(--gray) }
  ...
  .sn-active { background: white; border: 2px solid var(--brand); color: var(--brand); }
  .sn-locked { background: var(--gray-); color: #374151 }

  .step-meta { flex: 1; min-width: 0; }
  .step-meta h3 { font-size: 14px; font-weight: 500; margin-bottom: 1px; }
  .step-meta p { font-size: 12px; color: var(--gray-dark); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .b-done { background-color: var(--green); color: white; }
  .b-active { background: var(--brand-bg); color: var(--brand); border: 1px solid var(--brand-bg); }
  .b-locked { background: var(--gray-dark); }
  .b-waiting { background: var(--amber-bg); color: var(--amber); }

  .dropdown {
    font-size: 16px; color: var(--gray-dark);
    transition: transform 0.2s; display: inline-block; line-height: 1;
  }
  .step.is-open .dropdown { transform: rotate(180deg); }
  ...
  .screening-info { font-size: 12px; color: #374151; line-height: 1.7; }
  .screening-info strong { font-weight: 500; }

  .info-row {
    display: flex; align-items: flex-start;
    gap: 10px; padding: 10px 12px; border-radius: 8px;
  }
  .info-row.green { border: 1px solid var(--green-border); }
  .info-row.blue { background: var(--blue-bg); border: 1px solid var(--blue-border); }
  .info-row-icon { font-size: 18px; margin-top: 1px; flex-shrink: 0; }
  .info-row.green .info-row-icon { color: var(--green); }
  .info-row.blue .info-row-icon { color: #2563eb; }
  .info-row-text p { font-size: 13px; font-weight: 500; margin-bottom: 2px; }
  .info-row-text small { font-size: 11px; color: var(--gray-dark); }
  .info-row.blue .info-row-text p { font-size: 12px; font-weight: 400; color: #1e40af; line-height: 1.5; }

  .doc-list { display: flex; flex-direction: column; gap: 6px; margin-bottom: 12px; }
  .doc-row {
    display: flex; align-items: center; gap: 10px;
    padding: 9px 12px; border: 1px solid var(--gray);
  ...
  .di-done { color: var(--green); }
  .di-wait { background: var(--amber-bg); color: var(--amber); }
  .di-lock { background: var(--gray); color: var(--gray-dark); }
  .doc-name { flex: 1; font-size: 13px; }
  .doc-status { font-size: 11px; }
  .ds-done { color: var(--green); }
  .ds-wait { color: var(--amber); }
  .ds-lock { color: var(--gray-dark); }
  ...
  .upload-zone:hover { border-color: var(--brand); background: var(--gray); }
  .upload-zone p { font-size: 12px; color: var(--gray-dark); }
  .upload-zone small { font-size: 11px; color: var(--gray-dark); }

  @media (max-width: 480px) {
    .step-meta p { display: none; }
  }
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="steps">
    <div class="step">
      <div class="step-header" onclick="toggle(this)">
        ...
        <div class="step-right">
          <span class="badge b-done"><i class="fa fa-check"></i> Complete</span>
          <span class="dropdown"><i class="fa fa-chevron-down"></i></span>
        </div>
      </div>
      <div class="step-body">
        <div class="confirmed-row"><i class="fa fa-file-alt"></i> resume_juan_dela_cruz_v3.pdf uploaded · Apr 10</div>
      </div>
    </div>
    ...
    <div class="step">
      ...
      <div class="step-body">
        <div class="confirmed-row"><i class="fa fa-paper-plane"></i> Application submitted to DevTech · Apr 14</div>
      </div>
    </div>
    ...
    <div class="step">
      ...
      <div class="step-body">
        ...
        <div class="confirmed-row" style="margin-top:10px;"><i class="fa fa-check-circle"></i> Marked attended by Adviser Santos · Apr 22</div>
      </div>
    </div>
    ...
    <div class="step is-active is-open">
      <div class="step-header" onclick="toggle(this)">
        ...
        <div class="step-right">
          <span class="badge b-active">In progress</span>
          <span class="dropdown"><i class="fa fa-chevron-down"></i></span>
        </div>
      </div>
      <div class="step-body">
        ...
        <div class="upload-zone" onclick="handleUpload()">
          <i class="fa fa-upload" style="font-size:24px;display:block;margin:0 auto 6px;color:#9ca3af;"></i>
