            ...
            <div class="profile-container mx-auto mt-5">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-3 align-items-center">
                        <div class="col-md-8">
                            <label class="form-label">Upload Resume <span style="color:#aaa;font-weight:400;">(PDF, max
                                    5MB)</span></label>
                        ...
                        <div class="col-md-4">
                            ...
                            <?php else: ?>
                                <p class="text-muted mt-3" style="font-size:13px;">No resume uploaded yet.</p>
                            <?php endif; ?>
                        </div>
                    </div>

                    <div class="section-label mt-3">Credentials</div>

                    <!-- Existing credentials grid -->
                    <?php if (!empty($credentials)): ?>
                        <div class="row g-2 mb-3">
                            <?php foreach ($credentials as $cred): ?>
                                <div class="col-6 col-md-3 position-relative">
                                    <img src="../uploads/credentials/<?= htmlspecialchars($cred['credential_path']) ?>"
                                        class="img-fluid rounded"
                                        style="height:100px;width:100%;object-fit:cover;border:1px solid #eee;" alt="Credential">
                                    <form method="POST" style="position:absolute;top:4px;right:4px;">
                                        <input type="hidden" name="delete_credential" value="<?= $cred['id'] ?>">
                                        <button type="submit" title="Delete"
                                            style="background:#e74c3c;border:none;border-radius:50%;width:22px;height:22px;
                                                   color:#fff;font-size:11px;line-height:22px;padding:0;cursor:pointer;"
                                            onclick="return confirm('Delete this credential?')">
                                            <i class="fa fa-times"></i>
                                        </button>
                                    </form>
                                    <div style="font-size:10px;color:#aaa;margin-top:3px;">
                                        <?= date('M d, Y', strtotime($cred['uploaded_at'])) ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <p class="text-muted mb-3" style="font-size:13px;">No credentials uploaded yet.</p>
                    <?php endif; ?>

                    <!-- Upload new credentials -->
                    <div class="row g-3 align-items-center">
                        <div class="col-md-12">
                            <label class="form-label">Upload Credential Images
                                <span style="color:#aaa;font-weight:400;">(JPG/PNG, max 5MB each — multiple allowed)</span>
                            </label>
                            <input type="file" name="credentials[]" class="form-control" accept=".jpg,.jpeg,.png" multiple>
                        </div>
                    </div>

                    <div class="text-end mt-4">
                        <button type="submit" class="btn-save">
                            <i class="fa fa-upload me-1"></i> Upload Documents
                        </button>
                    </div>
                </form>
